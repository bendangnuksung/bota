...
class LogStat():
    ...
    def get_command_calls(self, n=7):
        ...
        for p in ax.patches:
            ax.annotate(str(p.get_height()), (p.get_x() * 1.005, p.get_height() * 1.005))
        path = os.path.join(constant.COMMAND_CALLS_IMAGE_PATH)
        plt.savefig(path)
        plt.close()
        plt.clf()
        return constant.COMMAND_CALLS_IMAGE_PATH, summary
    ...
    def get_commands_stats(self, n=7):
        ...
        ax[1].legend(loc=2)
        path = os.path.join(constant.COMMAND_STATS_IMAGE_PATH)

        plt.savefig(path)
        plt.close()
        plt.clf()
        return constant.COMMAND_STATS_IMAGE_PATH, summary
